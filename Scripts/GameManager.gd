...
func check_game_win():
	if goals_met() and board_stable:
		...
		if level == 19 && GameDataManager.level_info[level]["pass"] == 2:
			GameDataManager.level_info[level]["pass"] = 2
		else:
			GameDataManager.level_info[level]["pass"] = 1
			
		if GameDataManager.level_info.size() == level:
...
func _on_grid_update_score(streak_value):
	...
	if current_score >= max_score:
		GameDataManager.level_info[level]["stars unlocked"] = 1
	if current_score >= GameDataManager.level_info[level]["high score"]:
		GameDataManager.level_info[level]["high score"] = current_score
...
func _on_concrete_holder_break_concrete(goal_type):
	check_goals(goal_type)

func _on_slime_holder_break_slime(goal_type):
	check_goals(goal_type)

func _on_lock_holder_break_lock(goal_type):
	check_goals(goal_type)
